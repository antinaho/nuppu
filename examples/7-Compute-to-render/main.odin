...
_init :: proc() {
    ...
    state.compute_pso = gpu.compute_pipeline_init(kernel, "mandelbrot_set")
...
_render :: proc(current: ^State, alpha: f32) {
    ...
    // Dispatch the compute pass, then read the result in the render pass.
    tex, tex_ok := nuppu.get_texture(current.texture)
    assert(tex_ok, "7-Compute-to-render: failed to get compute texture")

    gpu.set_compute_pipeline(current.compute_pso)
    compute_block := gpu.Parameter_Block {
        constants            = { 0 = current.animation_uniform },
