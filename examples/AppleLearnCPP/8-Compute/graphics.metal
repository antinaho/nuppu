...
struct Vertex_Data {
    float4 position; // .xyz = position, .w = 1.0
    float4 normal;   // .xyz = direction, .w = 0
    float2 texcoord;
};
...
v2f vertex vertexMain(device const Vertex_Data*   vertex_data   [[buffer(0)]],
    ...
    o.position = camera_data.perspective_transform * camera_data.world_transform * id.transform * vd.position;

    float3 normal = id.normal_transform * vd.normal.xyz;
