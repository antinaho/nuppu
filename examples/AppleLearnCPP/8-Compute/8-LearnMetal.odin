...
init :: proc() {
    window_app.v_shader = nuppu.shader_init(#load("8-graphics.metal", []u8), "vertexMain", .Vertex)
    window_app.f_shader = nuppu.shader_init(#load("8-graphics.metal", []u8), "fragmentMain", .Fragment)

    window_app.kernel = nuppu.shader_init(#load("8-compute.metal", []u8), "mandelbrot_set", .Compute)
