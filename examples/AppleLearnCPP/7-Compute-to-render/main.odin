...
pad_zero :: proc(v: [3]f32) -> [4]f32 {
    return [4]f32{ v[0], v[1], v[2], 0 }
}
...
when ODIN_OS == .JS {
    ...
    for &v, idx in vs {
        v.position = pad_zero(nuppu.UNIT_CUBE_VERTICES[idx])
        v.normal = pad_zero(nuppu.UNIT_CUBE_NORMALS[idx])
