...
@(private="file")
_make_manager :: proc(capacity: int = 8) -> ^nuppu.Entity_Manager {
    manager := new(nuppu.Entity_Manager)
    nuppu.entity_manager_init(manager)
    nuppu.entity_manager_add_variant(manager, Transform_Node, capacity)
    return manager
}
...
test_set_transform_writes_all_three_fields :: proc(t: ^testing.T) {
    ...
    node := nuppu.entity_add(manager, Transform_Node)
    ...
    e, _ := nuppu.entity_get(manager, node.handle)
...
test_set_transform_overwrites_previous_values :: proc(t: ^testing.T) {
    ...
    nuppu.entity_set_transform(manager, node.handle, {1, 1, 1}, {0, 0, 0}, {1, 1, 1})
    nuppu.entity_set_transform(manager, node.handle, {5, 6, 7}, {1, 1, 1}, {2, 3, 4})
...
test_set_transform_does_not_touch_prev_fields :: proc(t: ^testing.T) {
    ...
    nuppu.entity_set_transform(manager, node.handle, {9, 9, 9}, {0, 0, 0}, {2, 2, 2})
...
test_set_transform_stale_handle_is_noop :: proc(t: ^testing.T) {
    ...
    bogus := nuppu.Entity_Handle { index = 999, gen = 1, variant_idx = 0 }
...
test_move_increments_position :: proc(t: ^testing.T) {
    ...
    nuppu.entity_set_transform(manager, node.handle, {1, 2, 3}, {0, 0, 0}, {1, 1, 1})
    nuppu.entity_move(manager, node.handle, {10, 20, 30})
...
test_move_leaves_rotation_and_scale_alone :: proc(t: ^testing.T) {
    ...
    nuppu.entity_set_transform(manager, node.handle, {0, 0, 0}, {0.5, 0.5, 0.5}, {3, 3, 3})
    nuppu.entity_move(manager, node.handle, {1, 1, 1})
...
test_move_accumulates_across_calls :: proc(t: ^testing.T) {
    ...
    nuppu.entity_move(manager, node.handle, {1, 0, 0})
    nuppu.entity_move(manager, node.handle, {0, 2, 0})
    nuppu.entity_move(manager, node.handle, {0, 0, 4})
